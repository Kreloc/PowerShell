function Get-JavaVer
{
	...
	PROCESS
	{
		try
		{
			...
			$java = Get-ChildItem (Get-ChildItem (Get-ChildItem (Get-ChildItem "\\$ComputerName\c$\Program Files (x86)\Java\" | Where {$_.Name -like "jre1.*"}).FullName | Where {$_.Name -like "bin"}).FullName | Where {$_.Name -eq "java.exe"}).FullName
			$Version = $java.VersionInfo.FileVersion
			$props = @{JavaVersionInstalled=$Version
