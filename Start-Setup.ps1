...
#Script depends on four files: StartSetupScript.bat (Initials this PowerShell script), Start-Setup.ps1 (This script), count.txt (Keeps track of restarts and determines which piece of the script should run), 
and config.csv (Pipes ComputerName and Description to the script.). 
All of this files should be placed in a directory with the following path
C:\SystemTools\
This script accomplishes the following tasks: Names the Computer, populates the local computer's description field, adds the computer to the domain (Will prompt for domain admin credentials), disables IPv6 thru the registry, checks both the boxes in the IPv4 DNS
for 'Register this connection's address in DNS' & 'Use this connection's DNS suffix in DNS registration', downloads and installs Windows Updates from the Internet, downloads and installs Microsoft Updates from the Internet,
Adds the Power Users domain group to the local machine's Power Users group, Downloads and installs latest version of Flash (may need tweaking in future), Downloads and installs latest version of Reader (may need tweaking in future).
Computer will restart and autlogon a total of four times during this process.
#>
If(Test-Path "C:\SystemTools\"){}
else{New-Item -Type Directory -Path "C:\SystemTools\"}
$count = [int](Get-Content C:\SystemTools\count.txt)
If ($count -le '-1' -or $count -ge 5){$count = -1;$count > "C:\SystemTools\count.txt"}
$log = "C:\SystemTools\setuplog.txt"
$domain = "YOURDOMAINNAMEHERE"
###Set up script for initial computer installs.
...
.NOTES
    ...
    Author       : CrazyDave
    LastModified : 24 Mar 2015 4:59 PM; Modified by Kreloc
...
param(
	...
	###Below installs each executable with a Write-Progress bar.
	$total = 2
	$i = 0
	$folder = Get-ChildItem "C:\SystemTools\Installers\" | Select -ExpandProperty FullName
	$folder | foreach {
	$i++
	write-progress -activity "Installing Software" -CurrentOperation "Installing $_ .. $i of $total" -Status "Please be patient may take some time..." -PercentComplete (($i / $total) * 100)
    	Start-Process $_ -ArgumentList "/msi EULA_ACCEPT=YES /qn" -wait -passthru 
        }
...
If($count -eq 0)
	{
		Get-WUInstall -NotCategory "Language packs" -AutoSelectOnly -Verbose -IgnoreReboot
		$count + 1 > C:\SystemTools\count.txt
		Restart-Computer
	} #End if Count 0
##DO NOT EDIT
...
If($count -eq 1)
{
	...
	Get-WUInstall -NotCategory "Language packs" -MicrosoftUpdate -AutoSelectOnly -Verbose -IgnoreReboot
	$count + 1 > C:\SystemTools\count.txt
	Restart-Computer
} #End if Count 1
...
If($count -eq 3)
{
	...
	$group.add("WinNT://$domain/Power Users,group")
	###Clean up tasks
	Set-SecureAutoLogon -Username "Administrator"
	""
	...
	$Bin.Items() | ForEach {$_ | Remove-Item -Force}
	###Verification tasks
	#ComputerName
	...
	"Setup script has finished. Exiting"
	Exit
} #End if Count 3
##End Start Up Script